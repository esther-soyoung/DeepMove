...
class DataGowalla(object):
    def __init__(self, trace_min=10, global_visit=10, hour_gap=72, min_gap=10, session_min=2, session_max=10,
        ...
        self.TWITTER_PATH = tmp_path + 'gowalla_sort.txt'
        ...
        self.data = {}  # key: uid, val: list([pid, tim])
        self.venues = {}
        self.words_original = []
        self.words_lens = []
    ...
    def save_variables(self):
        dataset = {'data_neural': self.data_neural, 'vid_list': self.vid_list, 'uid_list': self.uid_list,
                    'parameters': self.get_parameters(), 'data_filter': self.data_filter,
                    'vid_lookup': self.vid_lookup}
        pickle.dump(dataset, open(self.SAVE_PATH + self.save_name + '.pk', 'wb'))
